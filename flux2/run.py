#!/usr/bin/env python3
"""Flux 2 end-to-end steering workflow.
...
re-runs only execute missing steps:
  ...
  2. Compute steering vectors  (text encoder only; freed after)
...
from .compute_vectors import collect_style_representations
from steering.elastic_band import ElasticBandConfig
from .elastic_band import ElasticBandFlux2Runner
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Flux 2 end-to-end steering workflow.")
    ...
    parser.add_argument("--model", type=str, default="black-forest-labs/FLUX.2-dev",
                        help="Flux 2 model ID for steering vector computation.")
    ...
    parser.add_argument("--config", type=Path, default=None,
                        help="Path to elastic-band YAML config. Auto-selected from concept type if omitted "
                             "(configs/flux2_local.yaml for Local Edit, configs/flux2_global.yaml otherwise).")
    return parser
...
def main() -> None:
    ...
    if args.config is not None:
        ...
        config_path = Path("configs/flux2_local.yaml" if concept_type == "Local Edit" else "configs/flux2_global.yaml")
        print(f"Auto-selected config: {config_path}")
    elastic_band_config = ElasticBandConfig.from_yaml(config_path)

    # -----------------------------------------------------------------------
    # Step 2: Compute steering vectors (text encoder only)
    # -----------------------------------------------------------------------
    ...
    if not vectors_done:
        ...
        vectors, labels = collect_style_representations(cv_args)
