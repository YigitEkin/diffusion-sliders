...
# Device mapping utilities from accelerate
try:
...
class Flux2Pipeline(DiffusionPipeline, Flux2LoraLoaderMixin):
    ...
    def __init__(
        ...
        self._distributed_load_config = None
    ...
    def from_pretrained_distributed(
        ...
        text_encoder_device_map: Optional[Union[str, Dict]] = "auto",
        transformer_device_map: Optional[Union[str, Dict]] = "auto",
        vae_device_map: Optional[Union[str, Dict]] = None,
        **kwargs,
    ):
        """
        Load the Flux2Pipeline with optimized device mapping across multiple GPUs.
        
        This method automatically distributes the model components across available GPUs
        to optimize memory usage and enable inference on models that don't fit on a single GPU.
        
        Args:
            pretrained_model_name_or_path (`str`):
                Path or HuggingFace Hub identifier for the pretrained model.
            torch_dtype (`torch.dtype`, *optional*, defaults to `torch.bfloat16`):
                The data type for the model weights.
            max_memory (`Dict[int, str]`, *optional*):
                Maximum memory per GPU device. Example: `{0: "40GB", 1: "40GB"}`.
                If not provided, uses balanced memory allocation.
            text_encoder_device_map (`str` or `Dict`, *optional*, defaults to `"auto"`):
                Device map for the text encoder. Use `"auto"` for automatic optimization.
            transformer_device_map (`str` or `Dict`, *optional*, defaults to `"auto"`):
                Device map for the transformer. Use `"auto"` for automatic optimization.
            vae_device_map (`str` or `Dict`, *optional*):
                Device map for the VAE. If None, VAE is loaded on the first available GPU.
            **kwargs:
                Additional arguments passed to `from_pretrained`.
                
        Returns:
            `Flux2Pipeline`: The pipeline with components distributed across GPUs.
        ...
        if not ACCELERATE_AVAILABLE:
            raise ImportError(
                "The `accelerate` library is required for distributed loading. "
                "Install it with: pip install accelerate"
            )
        
        from diffusers import Flux2Pipeline as BasePipeline
        
        # Get available GPU count
        num_gpus = torch.cuda.device_count()
        if num_gpus == 0:
            raise RuntimeError("No GPUs available for distributed inference")
        
        logger.info(f"Loading Flux2Pipeline distributed across {num_gpus} GPU(s)")
        
        # Set up max_memory if not provided - use balanced allocation
        if max_memory is None:
            max_memory = {}
            for i in range(num_gpus):
                # Reserve some memory for activations/intermediates
                props = torch.cuda.get_device_properties(i)
                total_memory = props.total_memory
                # Use 90% of available memory
                usable_memory = int(total_memory * 0.9)
                max_memory[i] = f"{usable_memory // (1024**3)}GB"
            # Also allow CPU offload as fallback
            max_memory["cpu"] = "100GB"
        
        logger.info(f"Memory allocation: {max_memory}")
        
        # Load text encoder with device mapping
        text_encoder = None
        if text_encoder_device_map is not None:
            logger.info("Loading text encoder with device_map...")
            text_encoder = Mistral3ForConditionalGeneration.from_pretrained(
                pretrained_model_name_or_path,
                subfolder="text_encoder",
                device_map=text_encoder_device_map,
                max_memory=max_memory,
                torch_dtype=torch_dtype,
            )
        
        # Load tokenizer (PixtralProcessor for Flux2)
        ...
        # Load transformer with device mapping
        logger.info("Loading transformer with device_map...")
        ...
        # Load VAE - typically smaller, can fit on single GPU
        logger.info("Loading VAE...")
        if vae_device_map is not None:
            ...
            # Load VAE on first available GPU
            vae = AutoencoderKLFlux2.from_pretrained(
                pretrained_model_name_or_path,
                subfolder="vae",
                torch_dtype=torch_dtype,
        ...
        pipe._distributed_load_config = {
            "pretrained_model_name_or_path": pretrained_model_name_or_path,
            "torch_dtype": torch_dtype,
            "max_memory": copy.deepcopy(max_memory),
            "text_encoder_device_map": copy.deepcopy(getattr(text_encoder, "hf_device_map", text_encoder_device_map)),
            "transformer_device_map": copy.deepcopy(getattr(transformer, "hf_device_map", transformer_device_map)),
            "vae_device_map": copy.deepcopy(getattr(vae, "hf_device_map", vae_device_map)),
        }
        
        logger.info("Flux2Pipeline loaded with distributed device mapping")
        return pipe
    
    def get_device_distribution(self) -> Dict[str, Any]:
        """
        Get the current device distribution of pipeline components.
        
        Returns:
            Dict with component names and their device assignments.
        """
        distribution = {}
        ...
        for name, module in [
            ...
            elif hasattr(module, "hf_device_map"):
                distribution[name] = module.hf_device_map
            else:
                # Get device from first parameter
                try:
                    first_param = next(module.parameters())
                    distribution[name] = str(first_param.device)
    ...
    @staticmethod
    def _module_device_summary(module: Optional[torch.nn.Module]) -> List[str]:
        if module is None:
            return []
    ...
    def _redistribute_fused_transformer(
        self,
        device_map: Optional[Dict[str, Union[str, int, torch.device]]] = None,
        force: bool = False,
    ) -> None:
        if not ACCELERATE_AVAILABLE:
            raise ImportError("`accelerate` is required to redistribute the fused transformer.")

        if self.transformer is None:
            raise ValueError("Pipeline does not have a transformer to redistribute.")

        if device_map is None:
            if self._distributed_load_config is None:
                raise ValueError("No distributed load configuration is available on this pipeline.")
            device_map = self._distributed_load_config.get("transformer_device_map")

        if not isinstance(device_map, dict):
            raise ValueError(
                "A concrete transformer device map is required for redistribution. "
                "Load the pipeline with `from_pretrained_distributed(...)` first."
            )

        if not force and getattr(self.transformer, "hf_device_map", None) == device_map:
            current_devices = self._module_device_summary(self.transformer)
            if sum(device.startswith("cuda:") for device in current_devices) > 1:
                return

        remove_hook_from_submodules(self.transformer)
        self.transformer = dispatch_model(self.transformer, device_map=device_map, force_hooks=True)

    def load_lora_weights_and_redistribute(
        self,
        ...
        **kwargs,
        ...
        original_device_map = copy.deepcopy(getattr(self.transformer, "hf_device_map", None))
        ...
        self.fuse_lora(
            ...
            adapter_names=[adapter_name] if adapter_name is not None else None,
        )
        self.unload_lora_weights()

        if redistribute:
            self._redistribute_fused_transformer(
                device_map=original_device_map,
                force=force_redispatch,
            )
    ...
    def _get_mistral_3_small_prompt_embeds(
        ...
        device = Flux2Pipeline._get_module_execution_device(text_encoder) if device is None else device
    ...
    def upsample_prompt(
        ...
        prompt = [prompt] if isinstance(prompt, str) else prompt
        device = self._get_module_execution_device(self.text_encoder) if device is None else device
